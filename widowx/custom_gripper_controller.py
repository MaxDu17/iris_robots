...
from widowx.srv import GetGripperState, GetGripperStateResponse
'''
...
class GripperControllerServer(GripperController):
    def __init__(self, robot_name, create_node=True, upper_limit=0.037, lower_limit=0.010, des_pos_max=1, des_pos_min=0):
        ...
        rospy.Service('get_gripper_state', GetGripperState, self.get_gripper_state_service)
    ...
    def get_gripper_state_service(self, req):
        return GetGripperStateResponse(self.get_continuous_position())



def run():
    controller = GripperControllerServer(robot_name='wx200s')
    rospy.sleep(0.1)
    r = rospy.Rate(50)
